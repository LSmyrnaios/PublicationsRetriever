...
public class UrlTypeChecker
{
	...
	private static final String docOrDatasetKeywords = "(?:file|pdf|document|dataset|article)";	// TODO - When adding more file-types, add the related exclusion here, as well.
	private static final String docOrDatasetNegativeLookAroundPattern = "(?<![\\w/_.,-]{0,100}" + docOrDatasetKeywords + "[\\w/_.,-]{0,100})(?!.*" + docOrDatasetKeywords + ".*)";
	...
	public static final Pattern URL_DIRECTORY_FILTER =
			Pattern.compile("[^/]+://.*/(?:(discover|profile|user|survey|index|media|theme|product|deposit|default|shop|view)/" + docOrDatasetNegativeLookAroundPattern	// Avoid blocking these if the url is likely to give a file.
					...
					+ "|(?:(?:advanced[-]?)?search|search/advanced|search-results|(?:[e]?books|journals)(?:-catalog)?|issue|docs|oai|(?:abstracting-)?indexing|online[-]?early|honors|awards|careers|meetings|calendar|diversity|scholarships|invo(?:ice|lved)|errata|classroom|publish|upload|products|forgot|home)[/]?$"	// Url ends with these.
	...
	public static boolean matchesUnwantedUrlType(String urlId, String retrievedUrl, String lowerCaseUrl)
	{
		...
		// Avoid pages based on unwanted url-string-content.
			...
			UrlUtils.logOutputData(urlId, retrievedUrl, null, "unreachable", loggingMessage, null, true, "true", wasUrlValid, "false", "false", couldRetry, null, "null");
			if ( !LoaderAndChecker.useIdUrlPairs )
