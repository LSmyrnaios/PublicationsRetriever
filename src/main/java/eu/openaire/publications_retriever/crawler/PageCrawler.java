...
public class PageCrawler
{
	...
	public static final Pattern NON_VALID_DOCUMENT = Pattern.compile(".*(?:[^e]manu[ae]l|(?:\\|\\|" + space + ")?gu[ií](?:de|a)|directive[s]?|preview|leaflet|agreement(?!.*thesis" + space + "(?:19|20)[\\d]{2}.*)|accessibility|journal" + space + "catalog|disclose" + space + "file|poli(?:c(?:y|ies)|tika(?:si)?)"	// "policy" can be a lone word or a word after: repository|embargo|privacy|data protection|take down|supplement|access
																		...
																		+ "|licen(?:se|cia)" + space + "(?:of|de)" + space + "us[eo]|(?:governance|safety)" + space + "statement|normativa|(?:consumer|hazard|copyright)" + space + "(?:information|(?:release" + space + ")?form)|permission|(?:editorial|review)" + space + "board|dé(?:p(?:ôt[s]?|oser)|butez)|créer" + space + "votre|orcid|subscription|instruction|code" + space + "of" + space + "conduct|request|join|compte|[^_]account"
																		...
																		+ "|(?:peer|mini)" + space + "review|(?:case|annual)" + space + "report|review" + space + "article|short" + space + "communication|letter" + space + "to" + space + "editor|how" + space + "to" + space + "(?:create|submit|contact)|tutori[ae]l|survey-results"
																		+ "|data-sharing-guidance|rate(?:" + space + ")?cards|press" + space + "release|liability" + space + "disclaimer|(?:avec|dans)" + space + "(?:ocd|x2)?hal|online(?:" + space + "|_)flyer|publishing" + space + "process|book" + space + "of" + space + "abstracts|academic" + space + "social" + space + "networks|ijcseugcjournalno"
																		+ "|^(?:licen[cs]e|help|reprints|pol[ií]ti[kc][sa](?:" + space + "de" + space + "informação)?|for" + space + "recruiters|charte" + space + "de" + space + "signature|weekly" + space + "visitors|publication" + space + "ethics" + space + "and" + space + "malpractice|redaktion)$"	// Single words/phrases inside the html-text.
																		+ "|/(?:entry|information|opinion|(?:rapportannuel|publerkl|utt_so_|atsc_|tjg_|oproep_voor_artikels_|[^/]*call_for_contributions_)[\\w-_()]*|accesorestringido|library_recommendation_form|research-article|loi_republique_numerique_publis|nutzungsbedingungen|autorenhinweise|mediadaten|canceledpresentations|sscc-facme_cirugia).pdf$"	// The plain "research-article.pdf" is the template provided by journals.
																		+ "|kilavuzu"	// "guide" in Turkish
																		+ "|(?:公表|登録)届出書|取扱要領|リポジトリ要項|検索のポイント|について|閲覧方法|ープンアクセスポリシー|されたみなさまへ|論文の許諾書).*");	// registration/notification form/statement, instructions, repository requirements, search point, how to browse (all in japanese), open access guide, to all of you, dissertation consent form
