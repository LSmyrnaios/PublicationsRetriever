...
public class DocUrlsRetriever
{
	...
	public static void showStatistics(Instant startTime, Instant finishTime)
	{
		...
		if ( MachineLearning.useMLA ) {
			logger.info("MLA is responsible for the discovery of " + MachineLearning.docUrlsFoundByMLA + " docUrls. That's about: " + MachineLearning.docUrlsFoundByMLA * (float)100 / UrlUtils.sumOfDocUrlsFound +"% from the total docUrls found. The MLA's average success-rate was: " + MachineLearning.getAverageSuccessRate() + "%");
		}
		...
		logger.debug("Αbout: " + UrlTypeChecker.elsevierUnwantedUrls * (float)100 / inputCheckedUrlNum + "% (" + UrlTypeChecker.elsevierUnwantedUrls + " urls) were from, or reached after redirects, the unwanted domain: \"elsevier.com\", which either doesn't provide docUrls in its docPages, or it redirects to \"sciencedirect.com\", thus being avoided to be crawled.");
		logger.debug("Αbout: " + UrlTypeChecker.doajResultPageUrls * (float)100 / inputCheckedUrlNum + "% (" + UrlTypeChecker.doajResultPageUrls + " urls) were \"doaj.org/toc/\" urls, which are resultPages, thus being avoided to be crawled.");
		logger.debug("Αbout: " + UrlTypeChecker.pagesWithHtmlDocUrls * (float)100 / inputCheckedUrlNum + "% (" + UrlTypeChecker.pagesWithHtmlDocUrls + " urls) were docUrls, but, in HTML, thus being avoided to be crawled.");
		...
		logger.debug("About: " + UrlTypeChecker.pagesNotProvidingDocUrls * (float)100 / inputCheckedUrlNum + "% (" + UrlTypeChecker.pagesNotProvidingDocUrls + " urls) were pages which are known to not provide docUrls.");
		...
		logger.debug("The number of domains blocked due to an \"SSL Exception\" was: " + HttpConnUtils.domainsBlockedDueToSSLException);
