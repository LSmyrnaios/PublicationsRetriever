...
public class PageCrawler
{
	...
	public static void visit(String urlId, String sourceUrl, String pageUrl, HttpURLConnection conn)
	{
		...
		if ( currentPageDomain == null ) {    // If the domain is not found, it means that a serious problem exists with this docPage and we shouldn't crawl it.
			...
			UrlUtils.logTriple(urlId, sourceUrl, pageUrl, "Discarded in PageCrawler.visit() method, after the occurrence of a domain-retrieval error.", null);
			return;
		}
		...
		try {
			...
			UrlUtils.logTriple(urlId, sourceUrl, "unreachable", "Discarded in PageCrawler.visit() method, as there was a problem retrieving its innerLinks. Its contentType is: \"" + pageContentType + "\"", null);
			return;
		}
		...
		if ( currentPageLinks.isEmpty() ) {	// If no links were retrieved (e.g. the pageUrl was some kind of non-page binary content)
			...
			UrlUtils.logTriple(urlId, sourceUrl, "unreachable", "Discarded in PageCrawler.visit() method, as no links were able to be retrieved from it. Its contentType is: \"" + pageContentType + "\"", null);
		...
		for ( String currentLink : currentPageLinks )
		{
            ...
            if ( UrlUtils.docUrls.contains(urlToCheck) ) {	// If we got into an already-found docUrl, log it and return.
				...
				if ( FileUtils.shouldDownloadDocFiles )
					UrlUtils.logTriple(urlId, sourceUrl, urlToCheck, "This file is probably already downloaded.", currentPageDomain);
				else
					UrlUtils.logTriple(urlId, sourceUrl, urlToCheck, "", currentPageDomain);
                return;
            }
            ...
            if ( UrlUtils.DOC_URL_FILTER.matcher(lowerCaseLink).matches() )
			{
				...
				try {
					...
					UrlUtils.logTriple(urlId, sourceUrl, "unreachable", "Logged in \"PageCrawler.visit()\" method, as an innerLink of this page caused \"ConnTimeoutException\".", null);
					return;
		...
		for ( String currentLink : remainingLinks )	// Here we don't re-check already-checked links, as they were removed.
		{
			...
			try {
				...
				UrlUtils.logTriple(urlId, sourceUrl, "unreachable", "Logged in PageCrawler.visit() method, as its domain was caught to not support the HTTP HEAD method.", null);
				return;
			} catch (ConnTimeoutException cte) {	// In this case, it's unworthy to stay and check other innerLinks here.
		...
		UrlUtils.logTriple(urlId, sourceUrl, "unreachable", "Logged in PageCrawler.visit() method, as no docUrl was found inside.", null);
