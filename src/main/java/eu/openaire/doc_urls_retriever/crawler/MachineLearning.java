...
public class MachineLearning
{
	...
	public static void gatherMLData(String docPage, String docUrl, String pageDomain)
	{
		...
		if ( extensionMatcher.find() ) {
			...
			//docUrlID = extensionMatcher.replaceAll("");	// Remove the extension.	TODO - Keep it here in case an idea comes up about using the IDs (but they can be non-equal: <docPageId != docUrlID>).
		}
	...
	public static synchronized boolean shouldRunPrediction()
	{
		...
		logger.debug("Breakpoint (urlNum=" + nextBreakPointForSuccessRate + ") reached. Current round's success rate of MLA = " + DocUrlsRetriever.df.format(curSuccessRate) + "%");
	...
	public static boolean predictInternalDocUrl(String urlId, String sourceUrl, String pageUrl, String pageDomain)
	{
		...
		for ( String knownDocUrlPath : knownDocUrlPaths )
		{
			...
			if ( UrlUtils.docOrDatasetUrlsWithIDs.containsKey(predictedDocUrl) ) {	// If we got into an already-found docUrl, log it and return true.
				...
				ConnSupportUtils.handleReCrossedDocUrl(urlId, sourceUrl, pageUrl, predictedDocUrl, logger, false);
